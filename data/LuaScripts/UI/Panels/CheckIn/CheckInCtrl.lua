...
CheckInCtrl.s_messages = HL.StaticField(HL.Table) << {
}

local CHECK_IN_PANEL_STATE_NAME = {
    PART_ONE = "PartOne",
    PART_TWO = "PartTwo",
}

local CHECK_IN_COLLECT_STATE_NAME = {
    LOCKED = "Locked",
    CAN_COLLECT = "CanCollect",
    COLLECTED = "Collected",
}

local CHECK_IN_RARE_STATE_NAME = {
    NORMAL = "Normal",
    SPECIAL = "Special",
}

local CHECK_IN_PANEL_ANIM_NAME = {
    SWITCH_TO_PART_ONE = "checkin_changeone",
    SWITCH_TO_PART_TWO = "checkin_changetwo"
}

local CHECK_IN_BTN_ANIM_NAME = {
    CAN_COLLECT = "checkinitem_receive",
    COLLECTED = "checkinitem_done",
    SP_CAN_COLLECT = "checkinitemsp_receive",
    SP_COLLECTED = "checkinitemsp_done",
}
...
CheckInCtrl.OnCreate = HL.Override(HL.Any) << function(self, args)
    ...
    local tabRedDotArgs1 = {
        checkInActivity = self.m_activity,
        firstDay = 1,
        lastDay = UIConst.CHECK_IN_CONST.PART_SPLIT_NUM
    }
    local tabRedDotArgs2 = {
        checkInActivity = self.m_activity,
        firstDay = UIConst.CHECK_IN_CONST.PART_SPLIT_NUM + 1,
        lastDay = self.m_maxCheckInDay
    }
    ...
    self.m_previewCharInfo = GameInstance.player.charBag:CreateClientPerfectGachaPoolCharInfo(charId)
    self.view.previewBtn.onClick:AddListener(function()
        CharInfoUtils.openCharInfoBestWay({
            initCharInfo = {
                instId = self.m_previewCharInfo.instId,
                templateId = self.m_previewCharInfo.templateId,
                isSingleChar = true,
                isTrail = false,
            }
        })
...
CheckInCtrl.SwitchPart = HL.Method(HL.Boolean, HL.Opt(HL.Boolean, HL.Boolean, HL.Boolean, HL.Boolean)) << function(self, showSecondPart, skipSwitchAnim, skipGradually, isInit, noScrolling)
    ...
    activeLayoutTime.endTime.text = Utils.appendUTC(Utils.timestampToDateYMDHM(endTime))
end
