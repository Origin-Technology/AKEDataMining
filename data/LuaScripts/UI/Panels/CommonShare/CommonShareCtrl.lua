...
CommonShareCtrl.OnCreate = HL.Override(HL.Any) << function(self, arg)
    ...
    self.view.saveBtn.onClick:AddListener(function()
        ...
        if not UNITY_EDITOR and currentPlatform == CS.UnityEngine.RuntimePlatform.PS5 then
            ...
            if _G.IFIX_PS5_CONTENT_EXPORT_FORCE_FINISH_ALL_REQUEST == nil then
                _G.IFIX_PS5_CONTENT_EXPORT_FORCE_FINISH_ALL_REQUEST = 1
            else
                _G.IFIX_PS5_CONTENT_EXPORT_FORCE_FINISH_ALL_REQUEST = _G.IFIX_PS5_CONTENT_EXPORT_FORCE_FINISH_ALL_REQUEST + 1
            end

            if _G.IFIX_PS5_CONTENT_EXPORT_FORCE_FINISH_ALL_REQUEST > 5 then
                for force_finish_index = 0, 4 do
                    local succ, _ = pcall(function()
                        CS.Unity.ContentManagement.PS5.ContentExport.ExportFinish(force_finish_index)
                    end)
                    if not succ then
                        logger.warn("PS5 force finish export request ID, but not found : " .. force_finish_index)
                    end
                end
                _G.IFIX_PS5_CONTENT_EXPORT_FORCE_FINISH_ALL_REQUEST = _G.IFIX_PS5_CONTENT_EXPORT_FORCE_FINISH_ALL_REQUEST - 5
            end
...
CommonShareCtrl._OnFadeInEnd = HL.Method() << function(self)
    ...
    if canShare then
        ...
        self.view.shareCellLayoutNaviGroup:SetFocusBindingText(Language["key_hint_common_select"])
